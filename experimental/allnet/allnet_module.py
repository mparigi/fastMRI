...
class AllnetModule(MriModule):
    """
    Allnet training module.

    Kunet, then Iunet, then Unet.
    """
    ...
    def forward(self, in_kspace, mask, crop_size):
        ...
        # # run through kunet
        kunet_output = self.kunet(in_kspace)

        # Data Consistency
        # print("mask.shape = ", mask.shape, "in_kspace.shape = ", in_kspace.shape, "unet_output.shape = ", unet_output.shape)        
        # out_kspace = unet_output
        out_kspace = torch.where(mask.type(torch.bool).squeeze(1).permute(0, 1, 3, 2), in_kspace, kunet_output)
        ...
        real_images = fastmri.complex_abs(complex_image_crop)

        improved_real_images = self.iunet(real_images)

        # apply Root-Sum-of-Squares bc multicoil data
        out_image = fastmri.rss(improved_real_images, dim=1)
...
class DataTransform(object):
    ...
    def __call__(self, kspace, mask, target, attrs, fname, slice_num):
        ...
        # normalize input
        _im, mean, std = transforms.normalize_instance(image, eps=1e-11)
