...
class KUnetSimpleModule(MriModule):
    """
    KUnetSimple training module.

    Unet but only operates on the k-space, after multiple kspaces have been consolidated (rss).
    """
    ...
    def forward(self, in_kspace, mask):
        ...
        in_kspace = in_kspace.unsqueeze(1) # add coil dimension (which is single)

        # unstack complex dimension in coil dimension
        in_kspace = torch.cat([in_kspace[:, :, :, :, i] for i in [0, 1]], 1)

        # in_kspace shape should be (batch_size, 2, width, height)
        ...
        out_kspace = torch.stack( torch.split(out_kspace, 1, dim=1), dim=4)
        ...
        real_image = real_image.squeeze(1)

        # import matplotlib
        # import matplotlib.pyplot as plt
        # matplotlib.use("TkAgg")
        # import numpy as np
        # plt.imshow(real_image[0].numpy(), cmap='gray')
...
class DataTransform(object):
    ...
    def __call__(self, kspace, mask, target, attrs, fname, slice_num):
        ...
        # check for FLAIR 203
        if masked_kspace.shape[-2] < crop_size[1]:
            crop_size = (masked_kspace.shape[-2], masked_kspace.shape[-2])

        cropped_kspace = transforms.complex_center_crop(masked_kspace, crop_size)
        ...
        if self.which_challenge == "multicoil":
            single_kspace = fastmri.rss(cropped_kspace)
        else:
            single_kspace = cropped_kspace


        # normalize input
        _im = fastmri.ifft2c(single_kspace)
        _im, mean, std = transforms.normalize_instance(_im, eps=1e-11)
        ...
        return single_kspace, target, mean, std, fname, slice_num, mask
